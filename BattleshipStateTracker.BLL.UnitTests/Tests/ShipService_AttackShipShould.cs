...
namespace BattleshipStateTracker.BLL.UnitTests.Tests
{
    ...
    public class ShipService_AttackShipShould
    {
        ...
        [TestMethod]
        public void AttackShip_ShouldThrowBusinessObjectNotFoundException_IfBoardDoesNotExists()
        {
            mock.Setup(repo => repo.GetBoard(It.Is<Guid>(id => id == ShipService_AttackShipShould_TestData.InvalidBoardId))).Returns((Board)null);

            Assert.ThrowsException<BusinessObjectNotFoundException>(() => shipService.AttackShip(ShipService_AttackShipShould_TestData.AttackShipRequestWithInValidBoardId));
        }
        ...
        [TestMethod]
        public void Attackship_ShouldReturnSunkAttackShipResultEnum_IfAllShipPositionsWereAttacked()
        {
            mock.Setup(repo => repo.GetBoard(It.Is<Guid>(id => id == ShipService_AttackShipShould_TestData.ValidBoardId))).Returns(ShipService_AttackShipShould_TestData.BoardWithExistingShipThatIsAlmostSunk);
            AttackShipResultEnum result = shipService.AttackShip(ShipService_AttackShipShould_TestData.AttackShipRequestWithHitCapability);

            Assert.AreEqual(AttackShipResultEnum.Sunk, result);
        }

        [TestMethod]
        public void Attackship_ShouldReturnWinAttackShipResultEnum_IfAllShipsWereSunk()
        {
            mock.Setup(repo => repo.GetBoard(It.Is<Guid>(id => id == ShipService_AttackShipShould_TestData.ValidBoardId))).Returns(ShipService_AttackShipShould_TestData.BoardWithOnlyOneShipThatIsAlmostSunk);
            AttackShipResultEnum result = shipService.AttackShip(ShipService_AttackShipShould_TestData.AttackShipRequestWithHitCapability);

            Assert.AreEqual(AttackShipResultEnum.Win, result);
        }
    }
}
